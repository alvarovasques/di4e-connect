    ...
    return (
      <TooltipProvider delayDuration={100}>
        <Tooltip>
          ...
          <TooltipContent>
            <p>Sentimento: {label} ({score.toFixed(2)})</p>
            {confidence && <p>Confiança: {(confidence * 100).toFixed(0)}%</p>}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
  ...
  return (
    <div className={cn("flex items-center gap-2 p-2 rounded-md bg-background border", small ? "text-xs" : "text-sm")}>
      ...
      <div className="flex flex-col">
        ...
        <span className="text-muted-foreground">Pontuação: {score.toFixed(2)}</span>
        {confidence && <span className="text-muted-foreground">Confiança: {(confidence * 100).toFixed(0)}%</span>}
      </div>
    </div>
  );
};
